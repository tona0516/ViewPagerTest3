...
public class DynamicFragmentPagerAdapter extends PagerAdapter{
	 ...
	 * DynamicFragmentPagerAdapter（or
	 * サブクラス）ではこのproxyを介してFragmentTransactionを制御する。
	...
	protected final class FragmentTransactionProxy {
		...
		/**
		 * FragmentTransactionの機能制限版Proxy DynamicFragmentPagerAdapter（or
		 * サブクラス）ではこのproxyを介してFragmentTransactionを制御する。
		 *
		 * @param ft
		 *            FragmentTransactionの実体
		 */
	...
	public final Object instantiateItem(ViewGroup container, int position) {
		 ...
		 *
		 * 1.今まで表示されていなかったFragmentを新規に表示する必要が出てきた 2.非表示になっていたけど再度表示する必要が出てきた
		 ...
		 *
		 * #getItemPosition(Object)が呼ばれるのはPagerAdapter#notifyDataSetChanged()
		 * が呼ばれたときだけである。（多分…）
		 */
	...
	/*
	 * @Override public Parcelable saveState() { Bundle state = new Bundle();
	 *
	 * for(int i = 0, size = _fragments.size(); i < size; i++) { FragmentInfo fi
	 * = _fragments.get(i); state.putString("name", fi.getName());
	 ...
	 *
	 * Bundle bundle = (Bundle)state; bundle.setClassLoader(loader);
	 *
	 * for(String key : bundle.keySet()) { if(!key.startsWith("f")) continue;
	 *
	...
	public final int getItemPosition(Object object) {
		 ...
		 * と言ってもViewPagerが持っているItem（object）が全部来るわけではなく、 現在表示されているItemだけがやってくる。
		 *
	 ...
	 *
	 * @param position
	 *            0から始まる挿入する位置（元々あったFragmentは右にずれる）
	 * @param name
	 *            表示名
	 * @param fragment
	 *            挿入するFragment
